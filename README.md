...
- `--ice-servers` / `ICE_SERVERS` — the ICE servers to build the peer
  ...
  uses host candidates only for a direct LAN connection, without STUN or
  TURN.
...
| `--gripper-open-angle-left` | `GRIPPER_OPEN_ANGLE_LEFT` | `1.57`   | The left gripper's angle in radians while its trigger is released. |
| `--gripper-closed-angle-left` | `GRIPPER_CLOSED_ANGLE_LEFT` | `-0.4` | The left gripper's angle in radians with its trigger fully pulled, which is how far in that hand grips. Past zero asks for more than the closed jaws can reach, so the grip holds an object rather than touching it; short of zero stops the jaws apart. The trigger interpolates between this and the open angle. |
| `--gripper-open-angle-right` | `GRIPPER_OPEN_ANGLE_RIGHT` | `-1.57` | The right gripper's angle in radians while its trigger is released. The sign is opposite the left one because the two grippers open across opposite ends of the same axis. |
| `--gripper-closed-angle-right` | `GRIPPER_CLOSED_ANGLE_RIGHT` | `0.4` | The right gripper's angle in radians with its trigger fully pulled. Each hand's travel is given on its own, so the two need not match. |
